...
为避免正在运行的 MySQL、Redis 和应用容器与 Docker 构建争抢内存，专用入口会先停止这三个容器，因此部署期间会有一个短暂维护窗口。镜像构建、插件校验或容器切换失败时（包括用户按下 `Ctrl+C`），脚本会重新启动原有容器；新镜像成功启动后则按正常流程等待健康检查。进程被操作系统强制终止或服务器断电时无法执行自动恢复，此时可使用 `python3 scripts/restart.py --restart-only` 启动原有容器。
