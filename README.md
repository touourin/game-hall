...
象棋与围棋 AI 会随正式 Docker 镜像一起构建，容器运行时无需联网；本地开发也可在未安装外部引擎时使用内置降级棋手。版本、资源限制、环境变量和许可证说明见 [本地棋类 AI](docs/AI_ENGINES.md)。
...
当前版本定位为可联网测试的第一版：中国象棋已经限制三次重复局面，但尚未单独区分长将与长捉；围棋采用服务端中国面积数子，由双方标记死子并确认结果；斗地主暂未加入倍数、春天、计分和托管；军旗采用双人 12×5 棋盘，不包含四国军旗、裁判模式和计时赛。房间快照会持久化到 Redis，已结束对局的战绩写入 MySQL。
