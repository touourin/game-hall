...
- 十一款游戏共用访问密码、个人账号、登录会话与三套全局界面皮肤：墨玉会所、午夜铬光和象牙棋院；界面皮肤与棋盘、扑克皮肤相互独立
- 账号名固定用于登录；游戏昵称统一用于大厅、对局、聊天与排行榜，设置中可每 30 天修改一次，旧昵称仍归原账号保留
- 通过访问密码后可使用 7 天有效的游客席位，无需注册即可创建、加入、主持和完成游戏，并支持断线重连
- 多人房间可选择允许游客或仅限登录玩家；只要开局阵容包含游客，整局均为休闲局，不写入任何参与者的个人战绩、历史或排行榜
- 每款游戏均可查看独立战绩和排行榜，也可在大厅查看全部战绩
- 十一款游戏统一使用 `ArcadeHome`、`ArcadeRoom`、`useArcadeStore` 与 `arcade:*` 实时协议；各游戏只维护自己的规则引擎和专属对局界面
