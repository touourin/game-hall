...
- 内置游戏与社区插件游戏共用个人账号、登录会话和四套全局界面皮肤：橘光晴釉、幽蓝冷钢、月白云瓷和曜石黑钛；界面皮肤与棋盘、扑克皮肤相互独立
...
- 官方游戏与已发布的社区插件进入同一游戏注册表，统一使用 `ArcadeHome`、`ArcadeRoom`、`useArcadeStore` 与 `arcade:*` 实时协议；各游戏只维护自己的规则引擎和专属对局界面
...
- 围棋：9/13/19 路棋盘、提子、禁入点、全局同形禁着、停一手、申请点目、死子标记、双方确认数子和可选贴目；与 AI 对局时点目申请自动通过，可添加三档 KataGo 离线 AI
- 德州扑克：2–8 人单桌淘汰赛，跨手继承筹码并轮换庄家，支持大小盲、翻牌/转牌/河牌、过牌/跟注/加注/全押、边池与摊牌结算
- 斗地主：逐张发牌、三人叫抢、随时明牌、地主底牌、完整牌型与倍数结算；经典和不洗牌玩法配置 DouZero 后可添加模型 AI 玩家
...
主项目可以在未初始化社区游戏子仓库时独立运行。生产环境或需要开发社区游戏时，再执行第二条命令启用插件：
...
未初始化 Submodule 时，大厅只包含官方游戏；已初始化后，构建和启动会自动读取社区仓库的 `registry.json`。空目录或目录不存在是合法状态，但只要目录中存在文件，注册表和已发布插件就必须完整通过校验。前端插件模块清单属于构建产物，不提交到主仓库，`dev`、`test` 和 `build` 会在运行前自动生成。
...
以上命令可以直接运行仅含官方游戏的主项目。需要社区游戏时，在启动前额外执行 `git submodule update --init --recursive`。
