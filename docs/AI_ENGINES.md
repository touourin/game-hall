...
DouZero 依赖与权重按需选装。容器启动后不需要联网。AI 进程按需启动并复用；计算
超时或进程退出时，服务端会使用各游戏的安全兜底动作完成当前回合，房间不会卡死。
...
Compose 会把 `DOUZERO_MODEL_HOST_DIR`（默认 `./ai/douzero`）只读挂载到容器中。
只有三个文件同时存在且模型预热成功时才开放 AI，避免半套或损坏权重进入房间。
