...
正式 Docker 镜像会在构建时下载、校验并编译 Pikafish 与 KataGo；仓库固定保存的
DouZero WP 权重及其运行依赖则按需打入同一个自包含镜像。容器启动后不需要联网，
也不依赖宿主机模型挂载。AI 进程按需启动并复用；计算超时或进程退出时，服务端会
使用各游戏的安全兜底动作完成当前回合，房间不会卡死。
...
为了让不使用斗地主 AI 的部署免于安装 PyTorch，默认 Docker 构建仍保留轻量运行时。
需要启用时，只需在 `.env` 设置 `INSTALL_DOUZERO_AI=1` 后重新构建镜像。官方
`douzero_WP` 三角色权重已经作为同一套版本保存在 `ai/douzero`，新服务器拉取仓库后
不需要再从网盘下载或手工复制模型；同一份权重同时适用于 `amd64` 和 `arm64`。
