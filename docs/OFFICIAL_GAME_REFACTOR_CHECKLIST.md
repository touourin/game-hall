...
- [x] 第二批运行时代码与测试新增 75 行、删除 161 行，净减少 86 行；372 项前端测试、生产构建、主题选择器和月白云瓷对比度检查全部通过。
...
- [x] 将多人启动页的 11 套官方游戏文案、强调色和辉光从中央映射迁入各游戏 `index.ts`；社区游戏继续使用共享回退，不强迫接入官方模块契约。
...
- [x] 全量验证通过：后端 564 项、社区模块边界 48 项、前端 375 项；生产构建、主题选择器和月白云瓷对比度检查通过。
...
- [x] 全量验证通过：后端 565 项、社区模块边界 48 项、前端 376 项；生产构建、主题选择器和月白云瓷对比度检查通过。

## 第六阶段：共享 UI 交互原语

本阶段按 `docs/moon-white-ui-system.md` 的实施顺序逐类迁移；每一批必须删除被替代的旧实现，不能只添加新组件后保留两套入口。

### 第一批：按钮原语

- [x] 新建 `UiButton`，统一 `primary`、`secondary`、`danger`、整行和紧凑尺寸；默认原生类型固定为 `button`，表单提交必须显式声明 `submit`。
- [x] 将账号入口、设置、大厅、游戏入口、共享房间、确认弹窗以及 11 款游戏中的 50 个通用按钮迁入同一个原语；游戏棋盘、选牌和阶段选择等专有操作继续保留独立组件与样式。
- [x] 删除 `styles.css` 中旧 `primary-button`、`secondary-button`、`danger-button`、`arcade-danger-button` 和 `wide-button` 的全部实现；源码与测试中不存在残留选择器。
- [x] 月白云瓷禁用态由原语直接负责，不再依赖覆盖十余种游戏选择器的中央补丁；电脑端标准高度 50px，390px 手机端整行按钮无横向溢出。
- [x] 新增原语契约测试，并同步所有行为测试使用稳定的新变体选择器；前端 378 项测试、生产构建、主题选择器和月白云瓷对比度检查通过。
- [x] 本批运行时/源码新增 297 行、删除 277 行，净增加 20 行；测试新增 71 行、删除 40 行，清单新增 15 行、删除 1 行；整批新增 383 行、删除 318 行，净增加 65 行。

### 第二批：图标与关闭按钮原语

- [x] 新建 `UiIconButton`，统一标准与紧凑尺寸，并继承平台的键盘焦点、禁用态和主题材质；默认原生类型固定为 `button`。
- [x] 将共享返回、设置、二维码、帮助、身份、提示关闭和弹窗关闭共 22 个纯图标操作迁入原语；带文字的 2 个战绩操作迁入 `UiButton`，没有强行混用两种语义。
- [x] 删除 `styles.css` 中旧 `header-action`、`icon-button` 和 `modal-close` 的实现与主题补丁；业务组件只保留弹窗定位类，不再重新声明按钮材质。
- [x] 棋盘落子、牌桌选择、单人方向键等具有独立尺寸或输入语义的游戏操作继续保留，不因外观近似强制复用。
- [x] 新增原语契约测试，并同步返回按钮测试使用稳定的新选择器；前端 379 项、后端 565 项、社区模块边界 48 项测试全部通过。
- [x] 再次全局审计所有“关闭/取消”原生按钮，将聊天、规则、画风、头像裁剪、棋谱和私密提示中遗漏的 9 个通用关闭操作迁入原语；只保留承担全屏点击区域职责的透明遮罩按钮。
- [x] 生产构建、类型检查、主题选择器和月白云瓷对比度检查通过；浏览器复核电脑端紧凑按钮为 34px、390px 手机端触摸区域为 44px，均无横向溢出。本批没有删除整文件，旧实现以全局样式及各组件重复圆形按钮样式的形式被删除。
- [x] 本批运行时/源码新增 160 行、删除 138 行，净增加 22 行；测试新增 21 行、删除 1 行，清单新增 12 行、删除 1 行；整批新增 193 行、删除 140 行，净增加 53 行。新增部分来自单一原语、手机端 44px 触摸契约及其测试，不是保留旧入口形成的双轨代码。

### 补充清理：静态无用代码

- [x] 使用严格未使用项检查复核官方前端，删除 8 个零调用导出和 6 个无用导入、计算值或模板引用；没有保留兼容转发层。
- [x] 逐个复核组件和模块入口；现有官方 Vue/TypeScript 文件均有运行时使用者，因此本批不为减少文件数强行删除仍承担职责的文件。
- [x] 保留动态生成的排行榜名次选择器、社区插件 SDK 公共入口和引擎回放测试接缝；这些不是可删除的死代码。
- [x] 社区游戏目录不在本批改动范围内；其插件自有类型告警不混入官方模块清理。
- [x] 运行时/源码新增 3 行、删除 54 行，净减少 51 行；测试删除 4 行；本批代码差异合计新增 3 行、删除 58 行，净减少 55 行。
- [x] 前端 379 项、后端 565 项、社区模块边界 48 项测试全部通过；生产构建、主题选择器和月白云瓷对比度检查通过。

### 补充清理：旧 UI 残留

- [x] 确认设置页只保留幽蓝冷钢、曜石黑钛、月白云瓷和橘光晴釉四套现行主题；早期 Nexus 大厅组件已在前序重写中删除。
- [x] 删除旧 Avalon 主题存储迁移和对应测试，不再维护已退出产品的主题入口。
- [x] 删除无模板使用者的旧 `game-page`、`switch`、`section-heading` 和 `player-list` 全局样式，并合并重复的 `attempt-badge` 声明。
- [x] 复核全部 Vue 内部样式；动态游戏色调、棋子状态、按钮变体和 `:deep()` 子组件覆盖均有运行时职责，未误删。
- [x] 本批代码新增 2 行、删除 93 行，净减少 91 行；没有新增替代层，也没有可安全整文件删除的现行 UI 组件。
- [x] 前端 378 项、后端 565 项、社区模块边界 48 项测试全部通过；生产构建、主题选择器和月白云瓷对比度检查通过。

## 第七阶段：官方后端模块特例收口

- [x] 将阿瓦隆整局落库、审计详情序列化和角色皮肤进度从 `AccountStore` 迁入 `games/avalon/records.py`；账号服务不再导入阿瓦隆 `Room`、`Role` 或能力常量。
- [x] 为 `GameRecords` 增加模块自有的战绩变体存储选择器；账号查询只解释通用 JSON 选择器，不再识别暗影梅林字段或阿瓦隆变体名称。
- [x] 将旧阿瓦隆房间修复和 pre-Arcade 房间迁移收归 `AvalonEngine`；共享实时恢复流程只调用可选引擎扩展点，不再导入或判断阿瓦隆类型。
- [x] 保留旧持久化数据的读取兼容，但兼容代码由对应游戏模块所有，没有在共享平台建立第二套阿瓦隆房间流程。
- [x] 新增战绩选择器和旧房间迁移所有权测试；后端 567 项、前端相关 41 项测试通过，生产构建、类型检查、主题选择器和月白云瓷对比度检查通过。

- [ ] 下一批先审计表单、分段选择和开关的实际重复边界，再决定 `UiField`、`UiSegmentedControl` 与 `UiToggle` 的最小接口；不先造空组件。
...
- [-] `gameCatalog.ts` 和 `gameRules.ts`：官方模块与社区插件之间的统一公共入口；只有替代入口完整覆盖后才能删除。
