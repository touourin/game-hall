...
- [x] 将共享返回、设置、二维码、帮助、身份、提示关闭和弹窗关闭共 13 个纯图标操作迁入原语；带文字的 2 个战绩操作迁入 `UiButton`，没有强行混用两种语义。
...
- [x] 生产构建、类型检查、主题选择器和月白陶瓷对比度检查通过；本批没有删除整文件，旧实现以 28 行全局样式及各组件重复样式的形式被删除。
- [x] 本批运行时/源码新增 122 行、删除 97 行，净增加 25 行；测试新增 21 行、删除 1 行，清单新增 11 行、删除 1 行；整批新增 154 行、删除 99 行，净增加 55 行。新增部分来自单一原语及其契约测试，不是保留旧入口形成的双轨代码。
