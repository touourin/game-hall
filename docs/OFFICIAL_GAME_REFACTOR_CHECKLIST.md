...
- [x] 月白陶瓷禁用态由原语直接负责，不再依赖覆盖十余种游戏选择器的中央补丁；电脑端标准高度 50px，390px 手机端整行按钮无横向溢出。
- [x] 新增原语契约测试，并同步所有行为测试使用稳定的新变体选择器；前端 378 项测试、生产构建、主题选择器和月白陶瓷对比度检查通过。
...
- [x] 生产构建、类型检查、主题选择器和月白陶瓷对比度检查通过；浏览器复核电脑端紧凑按钮为 34px、390px 手机端触摸区域为 44px，均无横向溢出。本批没有删除整文件，旧实现以全局样式及各组件重复圆形按钮样式的形式被删除。
...
- [x] 前端 379 项、后端 565 项、第三方模块边界 48 项测试全部通过；生产构建、主题选择器和月白陶瓷对比度检查通过。

### 补充清理：旧 UI 残留

- [x] 确认设置页只保留极光雾舱、暖钛陶瓷和月白陶瓷三套现行主题；早期 Nexus 大厅组件已在前序重写中删除。
...
- [x] 前端 378 项、后端 565 项、第三方模块边界 48 项测试全部通过；生产构建、主题选择器和月白陶瓷对比度检查通过。
...
- [x] 新增战绩选择器和旧房间迁移所有权测试；后端 567 项、前端相关 41 项测试通过，生产构建、类型检查、主题选择器和月白陶瓷对比度检查通过。
