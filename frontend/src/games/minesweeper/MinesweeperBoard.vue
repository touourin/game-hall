...
.minesweeper-mark { width: 44px; aspect-ratio: 1; display: grid; place-items: center; border-radius: 13px; color: #efaaa7; background: #a43f461b; }
.minesweeper-status > div:nth-child(2) { display: grid; gap: 3px; }.minesweeper-status > div:nth-child(2) small { color: var(--gold); font-size: 10px; font-weight: 850; }.minesweeper-status > div:nth-child(2) strong { font-family: "Songti SC", "STSong", serif; font-size: 20px; }
.minesweeper-status-data { min-width: 0; display: flex; gap: 7px; }.minesweeper-status-data > span { min-width: 82px; min-height: 39px; display: grid; grid-template-columns: auto auto; align-content: center; justify-content: center; column-gap: 5px; border: 1px solid var(--line); border-radius: 11px; padding: 4px 9px; background: #0002; }.minesweeper-status-data svg { color: var(--gold); }.minesweeper-status-data b { font-size: 14px; }.minesweeper-status-data small { grid-column: 1 / 3; color: var(--muted); font-size: 8px; text-align: center; }
.minesweeper-panel { padding: 14px; display: grid; gap: 12px; overflow: hidden; background: radial-gradient(circle at 50% 35%, #62c69b0d, transparent 52%), var(--surface); }
.minesweeper-panel.finished { border-color: color-mix(in srgb, var(--gold) 40%, var(--line)); }
...
.minesweeper-mode { display: flex; border: 1px solid var(--line); border-radius: 10px; overflow: hidden; }.minesweeper-mode button { min-height: 38px; display: inline-flex; align-items: center; gap: 5px; border: 0; padding: 0 11px; color: var(--muted); background: #0002; font-weight: 850; }.minesweeper-mode button + button { border-left: 1px solid var(--line); }.minesweeper-mode button.active { color: #10231d; background: var(--gold); }
.minesweeper-zoom { display: flex; align-items: center; border: 1px solid var(--line); border-radius: 10px; overflow: hidden; }.minesweeper-zoom button { width: 36px; min-height: 36px; display: grid; place-items: center; border: 0; color: var(--muted); background: #0002; }.minesweeper-zoom span { min-width: 48px; color: var(--gold); font-size: 10px; font-weight: 900; text-align: center; }
.minefield-viewport { min-width: 0; max-width: 100%; max-height: min(68vh, 720px); overflow: auto; border: 1px solid var(--line); border-radius: 14px; padding: 12px; background: color-mix(in srgb, var(--bg) 82%, black); box-shadow: inset 0 12px 30px #0005; overscroll-behavior: contain; scrollbar-color: color-mix(in srgb, var(--gold) 38%, transparent) transparent; }
...
.mine-cell { position: relative; width: var(--mine-cell-size); height: var(--mine-cell-size); min-width: 0; display: grid; place-items: center; border: 1px solid color-mix(in srgb, var(--line) 96%, white 5%); border-radius: clamp(5px, calc(var(--mine-cell-size) * .18), 9px); padding: 0; color: var(--text); background: linear-gradient(145deg, color-mix(in srgb, var(--surface-strong) 91%, white 5%), color-mix(in srgb, var(--surface-strong) 92%, black)); box-shadow: inset 1px 1px 0 #ffffff16, 0 2px 5px #0005; touch-action: pan-x pan-y; user-select: none; transition: transform .08s, border-color .12s, background .12s, opacity .12s; }
.mine-cell:not(:disabled) { cursor: pointer; }.mine-cell:active:not(:disabled) { transform: scale(.92); }.mine-cell:disabled { opacity: 1; }.mine-cell.pending { opacity: .62; }.mine-cell:focus-visible { z-index: 2; outline: 2px solid var(--gold); outline-offset: 1px; }
.mine-cell.state-open { border-color: color-mix(in srgb, var(--line) 52%, transparent); border-radius: 5px; background: color-mix(in srgb, var(--surface-soft) 52%, var(--bg)); box-shadow: inset 0 0 0 1px #0003; }.mine-cell.state-open b { font-size: clamp(13px, calc(var(--mine-cell-size) * .48), 21px); text-shadow: 0 1px 2px #0008; }.mine-cell.number-1 { color: #82b9e8; }.mine-cell.number-2 { color: #79d3a8; }.mine-cell.number-3 { color: #ef8c88; }.mine-cell.number-4 { color: #bc9de2; }.mine-cell.number-5 { color: #e0a06f; }.mine-cell.number-6 { color: #73d2d0; }.mine-cell.number-7 { color: #edf1ea; }.mine-cell.number-8 { color: #a7b8b2; }
.mine-cell.state-flagged { color: var(--gold); background: linear-gradient(145deg, color-mix(in srgb, var(--gold) 12%, var(--surface-strong)), var(--surface-strong)); }.mine-cell.state-mine { color: #efaaa7; background: #6f2d324c; }.mine-cell.state-exploded { z-index: 2; color: #fff; border-color: #f28a82; background: #a33f49; box-shadow: 0 0 0 4px #e85d6830, 0 0 24px #e85d6866; animation: mine-burst .38s ease; }.mine-cell.state-wrong_flag { color: #efaaa7; background: #5f34365c; }.wrong-flag { display: flex; align-items: center; font-weight: 950; }
@keyframes mine-burst { 45% { transform: scale(1.18) rotate(6deg); } }
.minesweeper-progress { height: 5px; overflow: hidden; border-radius: 999px; background: #0004; }.minesweeper-progress i { display: block; height: 100%; border-radius: inherit; background: linear-gradient(90deg, var(--gold), var(--green)); transition: width .18s ease; }
.minesweeper-panel > footer { min-width: 0; display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 10px; }.minesweeper-panel > footer p { min-width: 0; margin: 0; overflow: hidden; color: var(--muted); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.minesweeper-panel > footer button { min-height: 38px; display: inline-flex; align-items: center; gap: 6px; border: 1px solid var(--line); border-radius: 10px; padding: 0 11px; color: var(--muted); background: #0002; font-weight: 850; }
.minesweeper-result { padding: 24px; display: grid; justify-items: center; gap: 7px; text-align: center; }.minesweeper-result > span { display: flex; align-items: center; gap: 6px; color: #efaaa7; font-weight: 850; }.minesweeper-result.won > span { color: #8fe0bd; }.minesweeper-result h2 { margin: 2px 0 0; font-family: "Songti SC", "STSong", serif; font-size: clamp(32px, 7vw, 48px); }.minesweeper-result > p { margin: 0; color: var(--muted); }.minesweeper-result > div { width: min(100%, 500px); margin: 8px 0; display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; }.minesweeper-result > div span { border: 1px solid var(--line); border-radius: 11px; padding: 10px 6px; color: var(--muted); font-size: 10px; }.minesweeper-result > div b { display: block; margin-bottom: 3px; color: var(--gold); font-size: 15px; }.minesweeper-result .ui-button--primary { margin-top: 5px; }
