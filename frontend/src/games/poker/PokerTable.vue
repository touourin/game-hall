    ...
    <section class="poker-felt">
      <div class="opponent-grid">
        ...
        >
          ...
          <div class="mini-hand">
            <PlayingCard v-for="card in player.cards" :key="card.id" :rank="card.rankLabel" :suit="card.suitSymbol" :red="card.red" size="mini" />
            <PlayingCard v-for="index in player.cards.length ? 0 : player.cardCount" :key="`back-${index}`" size="mini" face-down />
          </div>
      ...
      <div class="community-area">
        <div class="community-cards">
          <PlayingCard v-for="card in game.communityCards" :key="card.id" :rank="card.rankLabel" :suit="card.suitSymbol" :red="card.red" />
          <PlayingCard v-for="slot in 5 - game.communityCards.length" :key="`empty-${slot}`" empty />
        </div>
      ...
      <article v-if="self" class="poker-seat self-seat" :class="{ acting: self.isActing, folded: self.folded, eliminated: self.eliminated }">
        <div class="self-hand">
          <PlayingCard v-for="card in self.cards" :key="card.id" class="own-card" :rank="card.rankLabel" :suit="card.suitSymbol" :red="card.red" />
        </div>
        <div class="self-copy">
...
<style scoped>
.poker-panel { display: grid; gap: 14px; }.poker-status { display: flex; align-items: end; justify-content: space-between; gap: 12px; }.poker-status div { display: grid; gap: 3px; }.poker-status small { color: var(--accent); font-weight: 850; letter-spacing: .08em; }.poker-status strong { display: flex; align-items: center; gap: 6px; font-size: 22px; }.poker-status > span { color: var(--muted); font-weight: 800; }
...
.mini-hand { display: flex; align-items: center; justify-content: center; }.mini-hand .playing-card + .playing-card { margin-left: -7px; }
...
.poker-controls { padding: 14px; display: grid; gap: 10px; }.poker-controls p { margin: 0; color: var(--muted); text-align: center; }.primary-actions { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 7px; }.primary-actions button, .raise-button, .quick-raises button { min-height: 44px; border: 1px solid var(--line); border-radius: 10px; color: var(--text); background: var(--surface-inset); font-weight: 850; }.primary-actions .fold { color: var(--red); }.primary-actions .all-in { color: var(--accent); border-color: color-mix(in srgb, var(--accent) 35%, var(--line)); background: color-mix(in srgb, var(--accent) 8%, var(--surface-inset)); }.raise-controls { display: grid; grid-template-columns: 1fr auto auto; align-items: end; gap: 8px; }.quick-raises { display: flex; gap: 5px; }.quick-raises button { min-height: 38px; padding: 0 9px; color: var(--muted); font-size: 12px; }.raise-controls label { display: grid; gap: 3px; color: var(--muted); font-size: 11px; }.raise-controls input { width: 100px; min-height: 38px; border: 1px solid var(--line); border-radius: 9px; padding: 0 9px; color: var(--text); background: var(--surface-inset); }.raise-button { padding: 0 13px; color: var(--accent-contrast); background: var(--accent); }
.next-hand-panel { display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 16px; }.next-hand-panel > div { display: grid; gap: 3px; }.next-hand-panel small { color: var(--accent); }.next-hand-panel span,.next-hand-panel p { margin: 0; color: var(--muted); }.next-hand-panel .ui-button--primary { flex: 0 0 auto; }
.poker-history { display: flex; flex-wrap: wrap; justify-content: center; gap: 5px; }.poker-history span { border: 1px solid var(--line); border-radius: 999px; padding: 4px 8px; color: var(--muted); font-size: 11px; }.poker-history b { margin-right: 4px; color: var(--text); }.poker-history em { margin-left: 3px; color: var(--accent); font-style: normal; }
