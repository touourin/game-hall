...
.junqi-status::after { content: ''; position: absolute; inset: 0 0 auto; height: 2px; opacity: 0; background: linear-gradient(90deg, transparent, var(--accent), transparent); transition: opacity .2s ease; }
.junqi-status > span { width: 42px; aspect-ratio: 1; display: grid; place-items: center; border: 1px solid color-mix(in srgb, var(--accent) 24%, transparent); border-radius: 9px; color: var(--accent); background: color-mix(in srgb, var(--accent) 9%, transparent); transform: rotate(45deg); }
.junqi-status > span :deep(svg) { transform: rotate(-45deg); }
.junqi-status strong, .junqi-status small { display: block; }.junqi-status small { margin-top: 3px; color: var(--muted); }.junqi-status em { color: var(--accent); font-style: normal; font-weight: 800; }
.junqi-status.active { border-color: color-mix(in srgb, var(--accent) 34%, transparent); }.junqi-status.active::after { opacity: 1; }
...
.junqi-cell.selected::after, .junqi-cell.latest::after, .junqi-cell.last-from::after { content: ''; position: absolute; z-index: 4; width: 88%; height: 75%; border: 3px solid var(--accent); border-radius: 7px; box-shadow: 0 0 0 2px color-mix(in srgb, var(--accent) 22%, transparent), 0 0 18px color-mix(in srgb, var(--accent) 55%, transparent); pointer-events: none; }
.junqi-cell.confirming::after { content: ''; position: absolute; z-index: 5; width: 88%; height: 75%; border: 3px solid var(--accent); border-radius: 7px; background: color-mix(in srgb, var(--accent) 14%, transparent); box-shadow: 0 0 0 3px color-mix(in srgb, var(--accent) 22%, transparent), 0 0 18px color-mix(in srgb, var(--accent) 58%, transparent); pointer-events: none; }
...
.terrain-legend { padding: 16px 18px; display: grid; gap: 11px; border-block: 1px solid var(--line); background: var(--surface-inset); }.terrain-legend h3 { margin: 0 0 2px; color: var(--muted); font-size: 11px; letter-spacing: .12em; }.terrain-legend > div { display: grid; grid-template-columns: 34px 1fr; gap: 10px; align-items: center; }.terrain-legend span b, .terrain-legend span small { display: block; }.terrain-legend span b { font-size: 12px; }.terrain-legend span small { margin-top: 2px; color: var(--muted); font-size: 10px; }
.legend-line { position: relative; display: block; width: 31px; height: 9px; }.legend-line::before { content: ''; position: absolute; top: 50%; left: 0; right: 0; border-top: 1px solid var(--muted); }.legend-line.railway { border-block: 2px solid var(--accent); }.legend-line.railway::before { border-color: transparent; background: repeating-linear-gradient(90deg, var(--accent) 0 2px, transparent 2px 6px); height: 100%; top: 0; }
.legend-node { justify-self: center; display: block; width: 18px; height: 18px; border: 2px double var(--accent); }.legend-node.camp { border-radius: 50%; }.legend-node.headquarters { width: 25px; height: 17px; border-style: solid; background: color-mix(in srgb, var(--accent) 12%, transparent); clip-path: polygon(12% 22%,50% 0,88% 22%,100% 22%,100% 100%,0 100%,0 22%); }
.junqi-side-panel details { padding: 15px 18px 18px; }.junqi-side-panel summary { display: flex; align-items: center; gap: 6px; color: var(--accent); cursor: pointer; font-weight: 800; }.junqi-side-panel p { margin-bottom: 0; color: var(--muted); font-size: 13px; line-height: 1.6; }
.junqi-actions { display: flex; justify-content: center; gap: 10px; }.junqi-actions button:not(.ui-button--danger) { padding: 11px 18px; display: inline-flex; align-items: center; gap: 7px; border: 1px solid var(--line); border-radius: 12px; color: var(--text); background: var(--surface); }.junqi-actions button.primary { color: var(--accent-contrast); background: var(--accent); }.junqi-actions button:disabled { opacity: .4; }
