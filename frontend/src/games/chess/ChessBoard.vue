...
<template>
  <section class="chess-panel">
    ...
    <div class="chess-actions">
      ...
      <button type="button" data-ui-interaction="lift" :disabled="!moveHistory.length" @click="exportMoves"><Download :size="17" />导出 PGN</button>
      <UiButton v-if="snapshot.phase === 'playing' && snapshot.actions.canAct" variant="danger" compact @click="arcade.action('resign')"><Flag :size="17" />认输</UiButton>
    </div>
    ...
    <div v-if="pendingPromotion" class="promotion-backdrop" @click.self="pendingPromotion = null">
      <section class="promotion-panel" role="dialog" aria-modal="true" aria-label="选择升变棋子">
        <header><div><small>兵抵达底线</small><strong>选择升变棋子</strong></div><UiIconButton compact aria-label="取消升变" @click="pendingPromotion = null"><X :size="20" /></UiIconButton></header>
        <div>
    ...
    <div v-if="showReplay" class="replay-backdrop" @click.self="showReplay = false">
      <section class="replay-panel" role="dialog" aria-modal="true" aria-label="国际象棋对局复盘">
        <header><div><small>标准棋谱</small><strong>第 {{ replayStep }} / {{ moveHistory.length }} 手</strong></div><UiIconButton compact aria-label="关闭复盘" @click="showReplay = false"><X :size="20" /></UiIconButton></header>
        <div class="move-list">
...
.chess-status { min-height: 28px; display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 7px 14px; color: var(--muted); text-align: center; }.chess-status strong { color: var(--accent); }.chess-status .selection { color: color-mix(in srgb, var(--accent) 78%, var(--text)); font-weight: 800; }.chess-status .check { border-radius: 999px; padding: 4px 8px; color: var(--accent-contrast); background: var(--red); font-weight: 900; }
...
.chess-cell::before,.chess-cell::after { position: absolute; content: ''; pointer-events: none; }.chess-cell.last-from::after { z-index: 1; inset: 0; background: color-mix(in srgb, #f7cf5c 32%, transparent); }.chess-cell.latest::after { z-index: 1; inset: 0; background: color-mix(in srgb, #f7cf5c 42%, transparent); box-shadow: inset 0 0 0 3px color-mix(in srgb, #fff1a8 64%, transparent); }.chess-cell.selected::after { z-index: 4; inset: 4px; border: 3px solid var(--accent); border-radius: 8px; background: color-mix(in srgb, var(--accent) 18%, transparent); box-shadow: inset 0 0 15px color-mix(in srgb, var(--accent) 25%, transparent), 0 0 16px color-mix(in srgb, var(--accent) 48%, transparent); }.chess-cell.legal::before { z-index: 4; width: 18%; aspect-ratio: 1; border-radius: 50%; background: #175e47b8; box-shadow: 0 0 0 5px rgba(23,94,71,.12); }.chess-cell.legal.capture::before { width: 78%; border: 4px solid #17694f; background: transparent; box-shadow: inset 0 0 0 3px rgba(229,255,243,.16); }.chess-cell.confirming::after { z-index: 5; inset: 3px; border: 3px solid #ffe48b; border-radius: 8px; background: rgba(255,220,104,.18); box-shadow: 0 0 18px rgba(255,220,104,.72); }.chess-cell.checked { box-shadow: inset 0 0 0 4px #d94742, inset 0 0 24px rgba(189,39,35,.7); }
...
.chess-actions { display: flex; flex-wrap: wrap; justify-content: center; gap: 8px; }.chess-actions > button:not(.ui-button--danger) { min-height: 42px; display: inline-flex; align-items: center; gap: 6px; border: 1px solid var(--line); border-radius: 10px; padding: 0 13px; color: var(--text); background: var(--surface); font-weight: 800; }.chess-actions button:disabled { opacity: .4; }
.promotion-backdrop,.replay-backdrop { position: fixed; z-index: 80; inset: 0; display: grid; place-items: center; padding: 18px; background: var(--backdrop); backdrop-filter: blur(7px); }.promotion-panel { width: min(92vw, 430px); display: grid; gap: 16px; border: 1px solid color-mix(in srgb, var(--accent) 40%, var(--line)); border-radius: 18px; padding: 18px; background: var(--surface-strong); box-shadow: var(--shadow); }.promotion-panel header,.replay-panel header { display: flex; justify-content: space-between; align-items: center; }.promotion-panel header > div,.replay-panel header > div { display: grid; }.promotion-panel header small,.replay-panel header small { color: var(--accent); }.promotion-panel > div { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 8px; }.promotion-panel > div button { min-width: 0; min-height: 96px; display: grid; place-items: center; gap: 3px; border: 1px solid var(--line); border-radius: 12px; color: var(--text); background: var(--surface-inset); }.promotion-panel > div span { font-family: "Times New Roman", serif; font-size: 48px; line-height: 1; }.promotion-panel > div span.white { color: #fff8df; text-shadow: 0 1px #4a3d31, 1px 0 #4a3d31, 0 -1px #4a3d31, -1px 0 #4a3d31; }.promotion-panel > div span.black { color: #222; text-shadow: 0 1px #ddd; }
.replay-panel { width: min(92vw, 560px); max-height: min(82vh, 720px); display: grid; grid-template-rows: auto 1fr auto; gap: 12px; border: 1px solid var(--line); border-radius: 17px; padding: 17px; background: var(--surface-strong); box-shadow: var(--shadow); }.move-list { min-height: 0; overflow: auto; display: grid; grid-template-columns: 1fr 1fr; gap: 7px; }.move-list button { min-height: 42px; display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 7px; border: 1px solid var(--line); border-radius: 8px; padding: 7px 9px; color: var(--text); background: var(--surface-inset); text-align: left; }.move-list button.active { border-color: var(--accent); background: color-mix(in srgb, var(--accent) 12%, var(--surface-inset)); }.move-list span { color: var(--muted); }.move-list b { font-family: ui-monospace, monospace; }.move-list em { color: var(--red); font-style: normal; font-size: 10px; }.replay-panel footer { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }.replay-panel footer button { min-height: 42px; display: flex; justify-content: center; align-items: center; gap: 5px; border: 1px solid var(--line); border-radius: 9px; color: var(--text); background: var(--surface-inset); }
@media (hover: hover) { .chess-cell:not(:disabled):hover .chess-piece { transform: translateY(-4px) scale(1.05); }.promotion-panel > div button:hover { border-color: var(--accent); transform: translateY(-2px); } }
@media (max-width: 600px) { .chess-panel { gap: 9px; }.chess-status { position: sticky; z-index: 7; top: max(6px, env(safe-area-inset-top)); width: 100%; border: 1px solid color-mix(in srgb, var(--accent) 28%, var(--line)); border-radius: 12px; padding: 8px 9px; background: color-mix(in srgb, var(--surface-elevated) 94%, transparent); box-shadow: 0 8px 24px rgba(0,0,0,.18); backdrop-filter: blur(14px); }.captured-pieces { gap: 5px; }.captured-pieces > span { min-height: 31px; padding: 4px 6px; }.captured-pieces b { font-size: 8px; }.captured-pieces i { font-size: 17px; }.chess-board { width: min(100%, calc(100dvh - 245px)); min-width: min(100%, 292px); border-width: 6px; border-radius: 10px; }.chess-piece { font-size: clamp(29px, 10.6vw, 51px); }.chess-actions { width: 100%; display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); }.chess-actions > button,.chess-actions > button:not(.ui-button--danger) { min-width: 0; justify-content: center; padding: 0 6px; font-size: 10px; }.move-list { grid-template-columns: 1fr; }.replay-panel { width: 100%; max-height: 88dvh; } }
@media (orientation: landscape) and (min-width: 601px) and (max-width: 980px) and (max-height: 600px) { .chess-panel { width: min(100%, 850px); grid-template-columns: minmax(0, 1fr) auto; grid-template-areas: "status board" "captured board" "actions board"; align-items: start; justify-items: stretch; gap: 9px 14px; }.chess-status { grid-area: status; align-self: end; border: 1px solid color-mix(in srgb, var(--accent) 28%, var(--line)); border-radius: 12px; padding: 9px 10px; background: color-mix(in srgb, var(--surface-elevated) 94%, transparent); }.captured-pieces { grid-area: captured; width: 100%; grid-template-columns: 1fr; gap: 6px; }.chess-board { grid-area: board; width: min(48vw, calc(100dvh - 76px)); min-width: 300px; max-width: 500px; }.chess-actions { grid-area: actions; width: 100%; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); }.chess-actions .ui-button--danger { grid-column: 1 / -1; } }
</style>
