...
<style scoped>
.solo-result-card { padding: clamp(22px, 4vw, 30px); display: grid; justify-items: center; gap: 8px; border-color: color-mix(in srgb, var(--result-tone, var(--green)) 28%, var(--line)); background: radial-gradient(circle at 50% 0, color-mix(in srgb, var(--result-tone, var(--green)) 10%, transparent), transparent 46%), var(--material-pattern), var(--surface); text-align: center; }
.solo-result-card.tone-danger { --result-tone: var(--red); }
.solo-result-card.tone-neutral { --result-tone: var(--gold); }
...
.solo-result-score { color: var(--text); font-family: "Songti SC", "STSong", serif; font-size: clamp(44px, 10vw, 68px); line-height: 1; }
.solo-result-score small { color: var(--gold); font-size: .3em; }
.solo-result-card h2 { margin: 3px 0 0; font-family: "Songti SC", "STSong", serif; font-size: clamp(30px, 6vw, 46px); }
