...
describe('坚持三秒确定性弹幕', () => {
  it('与服务端为相同种子生成相同弹幕', () => {
    expect(spawnBullets(123_456_789, 0)).toEqual([
      { x: 3_616, y: -120, vx: 59, vy: 165, radius: 44 },
      { x: 10_120, y: 1_340, vx: -134, vy: 41, radius: 44 },
    ])
    expect(spawnBullets(123_456_789, 73)).toEqual([
      { x: 8_593, y: 6_620, vx: -75, vy: -135, radius: 44 },
      { x: -120, y: 3_560, vx: 167, vy: 0, radius: 44 },
    ])
  })
  ...
  it('每帧根据方向输入移动玩家并生成弹幕', () => {
    ...
    expect(next.playerX).toBe(5_104)
  ...
  it('重放静止轨迹时在与服务端相同的帧发生碰撞', () => {
    ...
    expect(result.collisionTick).toBe(25)
    expect(result.tick).toBe(26)
  })
