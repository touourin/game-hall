...
.reaction-trigger { position: relative; width: min(72vw, 300px); aspect-ratio: 1; display: grid; place-items: center; align-content: center; gap: 10px; border: 1px solid color-mix(in srgb, var(--gold) 32%, transparent); border-radius: 48px; color: var(--gold); background: linear-gradient(145deg, color-mix(in srgb, var(--gold) 11%, transparent), color-mix(in srgb, var(--bg) 34%, transparent)), var(--surface-strong); box-shadow: inset 0 1px 0 color-mix(in srgb, var(--panel-highlight) 38%, transparent), var(--shadow-raised); cursor: pointer; overflow: hidden; transition: transform .14s; touch-action: manipulation; user-select: none; }
...
@keyframes waiting-pulse { 50% { border-color: color-mix(in srgb, var(--gold) 22%, transparent); box-shadow: inset 0 1px 0 color-mix(in srgb, var(--panel-highlight) 32%, transparent), 0 18px 45px color-mix(in srgb, var(--panel-shadow) 35%, transparent); } }
