...
.reaction-trigger { width: min(72vw, 300px); aspect-ratio: 1; display: grid; place-items: center; align-content: center; gap: 10px; border: 1px solid #e1bc6850; border-radius: 36%; color: var(--gold); background: linear-gradient(145deg, #e1bc681b, #06171955), var(--surface-strong); box-shadow: inset 0 1px 0 #ffffff14, 0 24px 55px #0007; cursor: pointer; transition: transform .14s, color .16s, border-color .16s, background .16s, box-shadow .16s; touch-action: manipulation; user-select: none; }
...
.reaction-trigger.ready { border-color: #a7efd0; color: #06231c; background: linear-gradient(145deg, #a7efd0, #62c69b); box-shadow: 0 0 0 10px #62c69b14, 0 24px 70px #3ab18442; animation: ready-pop .2s ease-out; }
...
@keyframes ready-pop { from { transform: scale(.94); } }
...
@media (prefers-reduced-motion: reduce) { .reaction-trigger.waiting, .reaction-trigger.ready { animation: none; } }
</style>
