...
const options: ArtworkSkinOption[] = [
  ...
  { id: 'deluxe', name: '典藏', description: '精致画风', tier: '高级', preview: '/deluxe.webp', items: [{ id: 'two', name: '角色二', group: '阵营二', artwork: '/two.webp', framing: { scale: 1.1, originXPercent: 50, originYPercent: 29, preserveFrame: true, treatment: 'codex-ink-wash' } }] },
]
...
describe('ArtworkSkinPicker', () => {
  it('previews caller-provided artwork before selecting it', async () => {
    ...
    expect(wrapper.get('.artwork-skin-gallery article').attributes('data-artwork-treatment')).toBe('codex-ink-wash')
