...
function winnerLabel(match: MatchDetail): string {
  ...
  if (match.gameKey === 'one_night_werewolf') {
  ...
  >
      ...
      <template v-if="selectedMatch">
        ...
        >
          ...
          <div class="match-court-timeline">
            <div>
              <strong>议庭结果</strong>
              <span>
                {{ selectedMatch.details.shadowMerlin.councilOpened ? '祓影议庭开启' : '祓影议庭未开启' }}
              </span>
            </div>
            <div v-if="selectedMatch.details.shadowMerlin.councilOpened">
              <strong>刺客选择</strong>
              <span>
                {{ selectedMatch.details.shadowMerlin.assassinationChosen ? '发动刺杀' : '放弃刺杀并结算祓影票' }}
              </span>
            </div>
            <div v-if="selectedMatch.details.shadowMerlin.exileTargetId">
              ...
              <em :class="selectedMatch.details.shadowMerlin.exileSuccess ? 'hit' : 'miss'">
                {{ selectedMatch.details.shadowMerlin.exileSuccess ? '祓影成功' : '祓影失败' }}
              </em>
            </div>
          </div>
        </div>
        ...
        <div v-if="selectedMatch.gameKey === 'survive_three_seconds'" class="match-detail-section">
          <span>三秒弹幕挑战</span>
          <div class="match-mission-list">
            <div :class="selectedMatch.winner === 'survived' ? 'success' : 'failed'">
              <strong>{{ selectedMatch.winner === 'survived' ? '完整生还' : '弹幕命中' }}</strong>
              <span>{{ formatDuration(selectedMatch.details.state?.elapsed_ms) }}</span>
              <small>目标 3.00 秒 · 60 Hz 轨迹重放</small>
            </div>
            <div class="success">
              <strong>服务端校验</strong>
              <span>{{ selectedMatch.details.state?.input_count ?? 0 }} 帧输入</span>
              <small>使用相同种子重建弹幕与碰撞位置</small>
            </div>
          </div>
        </div>
        ...
        <p class="match-detail-note">
          {{ selectedMatch.gameKey === 'reaction'
            ...
            : selectedMatch.gameKey === 'survive_three_seconds'
              ? selectedMatch.ranked ? '本次轨迹验证结果计入坚持三秒排行榜' : '本次挑战不计排行榜'
      ...
      <template v-else>
        ...
        <template v-else-if="summary">
          <div class="stats-summary-grid">
            ...
            <template v-else-if="props.gameKey === 'survive_three_seconds'">
              <div><strong>{{ summary.games }}</strong><span>挑战次数</span></div>
              <div><strong>{{ summary.wins }}</strong><span>成功生还</span></div>
              <div><strong>{{ summary.winRate }}%</strong><span>三秒存活率</span></div>
            </template>
          ...
          <div v-if="history.length" class="match-history-list">
            <button v-for="match in history" :key="match.id" type="button" @click="openMatch(match.id)">
              ...
              <span class="match-history-copy">
                ...
                <strong v-else-if="match.gameKey === 'survive_three_seconds'">{{ match.outcome === 'win' ? '坚持 3.00 秒' : match.reason }}</strong>
                ...
                <small v-else-if="match.gameKey === 'survive_three_seconds'">{{ formatDate(match.endedAt) }} · 180 帧轨迹已校验</small>
