...
`community`（社区游戏）永远是分类列表的最后一项。它是社区作品的统一入口和来源边界，不参与官方玩法分类之间的排序。

- 新增官方分类时，必须放在 `community` 之前；
- 不得按分类名称、游戏数量、活跃房间数或更新时间重新排列整个列表；
- 社区游戏数量发生变化时，不得改变它的末位位置；
- `buildGameCategories()` 应继续采用“全部官方分类在前，社区游戏最后追加”的返回结构。
...
- 分类色位置与基准色不变，只改变受光明暗；不得把分类色漂白成中性色；
- 不新增或删除结构，不重新生成另一套构图；
...
1. 在 `frontend/src/gameCategories.ts` 增加稳定英文 ID、名称、眉题、描述和收录规则，并将新增官方分类放在 `community` 之前。
...
9. 社区游戏仍是分类列表最后一项，未被动态排序移动。
10. 资源目录没有 SVG、PNG、草稿或孤立变体。
11. 测试和生产构建全部通过。
