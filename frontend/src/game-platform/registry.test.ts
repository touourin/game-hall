...
describe('built-in game registry', () => {
  it('owns the complete international chess integration metadata', () => {
    const definition = builtinGameDefinition('chess')

    expect(BUILTIN_GAME_DEFINITIONS).toHaveLength(1)
  ...
  it('returns null for games that still use the legacy path', () => {
    expect(builtinGameDefinition('gomoku')).toBeNull()
